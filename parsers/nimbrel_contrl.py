"""Nimbrel UN/EDIFACT D.01B CONTRL (Syntax and Service Report) builder + parser.

Pure functions, no Odoo. CONTRL is the interchange-level functional acknowledgement
exchanged between Nimbrel and suppliers: it syntactically acknowledges RECEIPT of an
interchange (it does NOT imply business acceptance). DE0083 action code ``8`` =
"interchange received" is the only value Nimbrel uses.

Worked example (Nimbrel_CONTRL.pdf p.8) — an ack returned to the supplier from
Nimbrel acknowledging receipt of interchange 72:
...
- Interchange parties: supplier = qualifier ``14`` (GLN), Nimbrel = qualifier ``ZZZ``.
...
The OUTBOUND CONTRL envelope itself is always supplier(=supplier_gln):14 -> NIMBREL:ZZZ
(MML acking Nimbrel). ``ctrl_ref`` is THIS CONTRL interchange's own control reference;
...
#: DE0083 — only "interchange received" is used by Nimbrel.
...
def build_contrl(payload: dict, *, supplier_gln: str = "SUPPLIER_GLN",
    ...
    # Envelope: supplier -> Nimbrel, CONTRL (no trailing ++++1).
    unb = build_unb(
...
def parse_contrl(text) -> dict:
    ...
    Per the Nimbrel CONTRL MIG, ``8`` (interchange received) is the ONLY
    action code Nimbrel uses — there is no documented negative/rejection
    ...
    Nimbrel-documented set, or a malformed UCI — is unambiguously flagged
