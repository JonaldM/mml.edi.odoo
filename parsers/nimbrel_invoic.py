"""Outbound UN/EDIFACT D.01B INVOIC (tax invoice) builder for Nimbrel.

Pure function (no Odoo). Builds the INVOIC interchange from a plain ``dict`` using
the shared :mod:`nimbrel_edifact` helpers + :class:`Segment` for the body, then
...
Message skeleton (segment order is build-critical)::
    ...
    UNA
    UNB  envelope (supplier:14 -> NIMBREL:ZZZ, business tail ++++1)
    ...
    GST semantics (Nimbrel_INVOIC.pdf p.52 Segment Group 27 + p.60-61 Segment
...
::

    {
      ...
      # --- parties ---
      "buyer": {                 # NAD BY
        "code": "NIMBREL",      # C082 party id (qualifier 92 = assigned by buyer)
...
double-checked against Nimbrel_INVOIC.pdf's own Segment Group 50 example
...
# EANCOM association assigned code carried in UNH S009/0057 for Nimbrel INVOIC.
...
def _nad(qualifier, party):
    """Build a NAD segment shaped like the Nimbrel worked example.
