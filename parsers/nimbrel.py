"""Nimbrel (NZ) EDI parser — UN/EDIFACT D.01B over SPS Commerce.
...
so a product keyed by MML's code OR the Nimbrel ISC resolves either way.
"""
from datetime import date

from .base_parser import BaseEDIParser, ParsedOrder, ParsedOrderLine, EDIParseError
from . import nimbrel_edifact as edifact
...
class NimbrelParser(BaseEDIParser):
    ...
    def generate_ack(self, review_record) -> bytes:
        ...
        from .nimbrel_ordrsp import build_ordrsp
    ...
    def generate_contrl(self, raw_text, partner) -> bytes:
        ...
        fresh control reference from the 'mml_edi.nimbrel.interchange.ref'
        ...
        """
        from .nimbrel_contrl import build_contrl, extract_unb_identity
    ...
    def build_outbound(self, msg_type, payload) -> bytes:
        ...
        if mt == "DESADV":
            from .nimbrel_desadv import build_desadv
            return build_desadv(payload)
        if mt == "INVOIC":
            from .nimbrel_invoic import build_invoic
            return build_invoic(payload)
        if mt == "CONTRL":
            from .nimbrel_contrl import build_contrl
            return build_contrl(payload)
        raise NotImplementedError("Nimbrel does not emit outbound %s" % msg_type)


class CancellationAckRefused(EDIParseError):
    """Raised by NimbrelParser.generate_ack when called on a cancellation
...
def _next_interchange_ref(partner):
    """Return the next Nimbrel interchange control reference (C2 sequence
    'mml_edi.nimbrel.interchange.ref'), or a fallback for the pure-test /
    no-env path.
...
def _qty_str(v) -> str:
    """Nimbrel quantities are eaches (integers on the wire)."""
    try:
...
def _gather_sol_by_line(review) -> dict:
    ...
    the same PO (mirrors kestrelby_idoc._gather_confirmations — finding #11).

    A multi-store Nimbrel interchange (or a partial-failure retry that split
...
def _review_to_ordrsp_payload(review) -> dict:
    ...
    if raw:
        try:
            ...
            for order in NimbrelParser().parse_file(data, partner):
