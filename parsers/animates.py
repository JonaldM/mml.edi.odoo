...
class AnimatesParser(BaseEDIParser):
    ...
    def parse_contrl(self, raw_text) -> dict:
        ...
        absent, treats the partner as having no CONTRL support. The
        implementation has always lived in parsers/animates_contrl.py, but
        while it was not exposed here every SPS acknowledgement resolved to
        None and the whole negative-CONTRL block was dead code. See
        _CONTRL_ACCEPTED_ACTIONS in the processor for the 7-vs-8 action codes.
        """
