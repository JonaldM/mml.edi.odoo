# mml.edi/parsers/kestrelby_idoc.py
"""
Kestrelby EDI Parser — SAP iDOC XML (ORDERS05 / CIMTYP ORDERSEXT).

This is the format Kestrelby actually sends MML over the EDIS VAN (NOT EDIFACT —
see kestrelby.py for the EDIFACT D96A variant). Parses ORDERS (new) and ORDCHG
(change) inbound iDOCs and generates ORDRSP (order response / ACK) iDOCs.

Ported from the proven .NET `KestrelbyEDI.iDocHandler` (GenerateEDIOrders /
...
iDOC structure (one <IDOC> per order document):
  ...
  E1EDP01    line items, repeating:
               ...
               E1EDP19 product ids: QUALF 003 = CASE/SHIPPER GTIN-14 (carton
                                    ...
                                    002 = MML internal code, 001 = Kestrelby buyer
...
QUALF 003 value is the CASE/SHIPPER GTIN-14 (Kestrelby uses separate shipper vs
...
buyer_article_no carries the Kestrelby article (QUALF 001) so the processor's match
...
# or rejected. "11" = "cannot supply" per the Kestrelby implementation guide / the
# legacy .NET handler.
...
class KestrelbyIDOCParser(BaseEDIParser):
    """Parser for Kestrelby SAP iDOC ORDERSEXT purchase orders + ORDRSP ACKs."""
    ...
    def parse_file(self, raw_content: bytes, trading_partner) -> list:
        ...
        _logger.info(
            "[KestrelbyIDOC] Parsed %d store order(s), %d total lines from iDOC",
    ...
    def _parse_idoc(self, idoc: ET.Element, raw_text: str) -> list:
        ...
        if mestyp == _MESTYP_RESPONSE:
            _logger.warning("[KestrelbyIDOC] ORDRSP (response) iDOC received inbound — skipping")
            return []
        ...
        for p01 in idoc.findall("E1EDP01"):
            ...
            # reduce to it (Kestrelby assigns some carton barcodes off an independent
    ...
    def generate_ack(self, review_record) -> bytes:
        ...
        # Per-PO ACK: Kestrelby expects ONE ORDRSP covering every store/line of the
        ...
        if len(set(belnrs)) > 1:
            ...
            if not matched:
                ...
                # confirm/reject lines Kestrelby never ordered on this PO.
                raise EDIParseError(
    ...
    def _gather_confirmations(self, review_record):
        ...
        try:
            ...
            # echo-all — that would tell Kestrelby we will fully supply lines we
    ...
    def _build_ordrsp(self, idoc, po_ref, confirmed, now, serial, has_review_data=False):
        ...
        #
        # OOS semantics per the Kestrelby iDOC ORDRSP IG v1.7 (docs/kestrelby.docs):
        ...
        for p01 in idoc.findall("E1EDP01"):
            ...
            if conf is None:
                if has_review_data:
                    ...
                    # never tell Kestrelby we will ship a line we never handled.
            ...
            # Kestrelby-accepted ORDRSP set WMENG in cartons; emitting the EA
