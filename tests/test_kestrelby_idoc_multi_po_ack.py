# mml.edi/tests/test_kestrelby_idoc_multi_po_ack.py
"""
Multi-IDOC (multi-PO) ORDRSP isolation tests for KestrelbyIDOCParser.
...
Run with:
    cd <module dir> && python -m pytest tests/test_kestrelby_idoc_multi_po_ack.py -v
"""
import copy
...
from mml_edi.parsers import kestrelby_idoc
...
class TestMultiPoAckIsolation:
    ...
    def test_ack_for_po_a_contains_only_po_a(self):
        out = KestrelbyIDOCParser().generate_ack(_review(PO_A, self.raw)).decode("utf-8")
    ...
    def test_ack_for_po_b_contains_only_po_b(self):
        out = KestrelbyIDOCParser().generate_ack(_review(PO_B, self.raw)).decode("utf-8")
        root = ET.fromstring(out)
        idocs = root.findall("IDOC")
        assert len(idocs) == 1
    ...
    def test_line_counts_match_own_po_only(self):
        ...
        for po in (PO_A, PO_B):
            root = ET.fromstring(KestrelbyIDOCParser().generate_ack(_review(po, self.raw)))
    ...
    def test_own_po_lines_confirmed_not_force_rejected(self):
        ...
        root = ET.fromstring(KestrelbyIDOCParser().generate_ack(_review(PO_A, self.raw)))
    ...
    def test_e1edp02_links_reference_own_po(self):
        root = ET.fromstring(KestrelbyIDOCParser().generate_ack(_review(PO_B, self.raw)))
    ...
    def test_multi_po_file_with_unmatched_po_fails_closed(self):
        ...
        with pytest.raises(EDIParseError):
            KestrelbyIDOCParser().generate_ack(_review("4500000000", self.raw))
...
class TestSinglePoUnchanged:
    ...
    def test_single_po_output_identical_to_unfiltered_echo(self, monkeypatch):
        ...
        monkeypatch.setattr(kestrelby_idoc, "datetime", _FrozenDatetime)
        parser = KestrelbyIDOCParser()
    ...
    def test_single_po_all_lines_still_echoed(self):
        root = ET.fromstring(KestrelbyIDOCParser().generate_ack(_review(PO_A, _load(SINGLE).decode("utf-8"))))
    ...
    def test_single_po_mismatched_po_ref_still_echoes(self):
        ...
        root = ET.fromstring(KestrelbyIDOCParser().generate_ack(
