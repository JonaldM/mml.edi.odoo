# mml.edi/tests/test_kestrelby_idoc.py
"""
Tests for the SAP iDOC XML parser (KestrelbyIDOCParser) against real Kestrelby
sample files. Pure-Python unit tests — no Odoo env required.

Run with:
    cd <module dir> && python -m pytest tests/test_kestrelby_idoc.py -v
"""
...
from mml_edi.parsers.kestrelby_idoc import KestrelbyIDOCParser
from mml_edi.parsers.base_parser import EDIParseError, ParsedOrder, ParsedOrderLine

FIXTURES = Path(__file__).parent / "fixtures"

SINGLE = "kestrelby_idoc_orders_single_7010168258.edi"   # ZNR single-store, 6 lines
MULTI = "kestrelby_idoc_orders_multi_4500176574.edi"     # ZNB bulk multi-store, 173 lines
...
class TestSingleStoreParsing:
    ...
    def setup_method(self):
        self.orders = KestrelbyIDOCParser().parse_file(_load(SINGLE), _partner())
...
class TestMultiStoreParsing:
    ...
    def setup_method(self):
        self.orders = KestrelbyIDOCParser().parse_file(_load(MULTI), _partner())
...
class TestParseEdgeCases:
    ...
    def test_ordrsp_is_skipped(self):
        ...
        assert KestrelbyIDOCParser().parse_file(ordrsp, _partner()) == []

    def test_invalid_xml_raises(self):
        with pytest.raises(EDIParseError):
            KestrelbyIDOCParser().parse_file(b"not xml <<<", _partner())
    ...
    def test_missing_po_number_raises(self):
        ...
        with pytest.raises(EDIParseError):
            KestrelbyIDOCParser().parse_file(bad, _partner())
    ...
    def test_change_order_detected_and_deleted_lines_dropped(self):
        ...
        orders = KestrelbyIDOCParser().parse_file(chg, _partner())
        assert len(orders) == 1
    ...
    def test_full_cancellation_yields_lineless_store_order(self):
        ...
        orders = KestrelbyIDOCParser().parse_file(cancel, _partner())
        assert len(orders) == 1
...
class TestGenerateAck:
    ...
    def test_returns_valid_ordrsp_xml(self):
        out = KestrelbyIDOCParser().generate_ack(_review())
    ...
    def test_response_reference_segment_added(self):
        root = ET.fromstring(KestrelbyIDOCParser().generate_ack(_review()))
    ...
    def test_short_supply_is_action_001_wmeng_carries_the_cut_no_abgru(self):
        ...
        root = ET.fromstring(KestrelbyIDOCParser().generate_ack(rv))
    ...
    def test_rejected_review_rejects_all_lines_003(self):
        root = ET.fromstring(KestrelbyIDOCParser().generate_ack(_review(state="rejected")))
    ...
    def test_summe_recalculated_when_short_echoed_when_full(self):
        ...
        full = ET.fromstring(KestrelbyIDOCParser().generate_ack(_review()))
        short = ET.fromstring(KestrelbyIDOCParser().generate_ack(
    ...
    def test_missing_raw_data_raises(self):
        ...
        r.edi_raw_data = None
        with pytest.raises(EDIParseError):
            KestrelbyIDOCParser().generate_ack(r)
...
class TestPerPoAck:
    ...
    def test_multistore_is_one_ordrsp_covering_all_stores(self):
        root = ET.fromstring(KestrelbyIDOCParser().generate_ack(_multi_review()))
...
class TestGatherConfirmationsErrorHandling:
    ...
    def test_orm_error_raises_instead_of_echo_all(self):
        ...
        with pytest.raises(RuntimeError):
            KestrelbyIDOCParser()._gather_confirmations(r)
    ...
    def test_no_env_path_echoes_full_supply(self):
        ...
        confirmed, has_review_data = KestrelbyIDOCParser()._gather_confirmations(r)
...
class TestRoundTrip:
    ...
    def test_generated_ack_is_skipped_if_reparsed(self):
        ack = KestrelbyIDOCParser().generate_ack(_review())
        # An ORDRSP fed back into parse_file must be ignored (not re-imported).
        assert KestrelbyIDOCParser().parse_file(ack, _partner()) == []
