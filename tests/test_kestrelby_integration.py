# mml.edi/tests/test_kestrelby_integration.py
"""
...
class EDIKestrelbySetup(EDITestSetup):
    """
    Extends EDITestSetup with Kestrelby-specific products and trading partner.
    Overrides self.trading_partner with Kestrelby config.
    """

    def setup_kestrelby_test_data(self):
        ...
        kestrelby_partner = self.env["res.partner"].create({
            "name": "Kestrelby Group Ltd",
            "customer_rank": 1,
        })
        self.env["res.partner"].create({
            "name": "Kestrelby Whangarei",
            "parent_id": kestrelby_partner.id,
            "ref": "1005",
        })
        self.env["res.partner"].create({
            "name": "Kestrelby Albany",
            "parent_id": kestrelby_partner.id,
            "ref": "1007",
        })

        # Override the generic trading_partner with Kestrelby-specific config
        self.trading_partner.write({
            "name": "Kestrelby Group Ltd",
            "code": "KESTRELBY",
            "partner_id": kestrelby_partner.id,
    ...
    def _run(self, content: bytes, filename="test.edi"):
        ...
        try:
            from odoo.addons.mml_edi.parsers.kestrelby import KestrelbyParser
        except ImportError:
...
class TestKestrelbyOrdersIntegration(EDIKestrelbySetup, TransactionCase):
    """Test ORDERS (BGM+220) new PO → sale order creation."""

    def setUp(self):
        super().setUp()
        self.setup_kestrelby_test_data()
...
@unittest.skipUnless(_ODOO_AVAILABLE, "Requires Odoo runtime")
@tagged("post_install", "-at_install")  # run after ALL modules load (e.g. website_sale's product fields)
class TestKestrelbyOrdchgIntegration(EDIKestrelbySetup, TransactionCase):
    ...
    def setUp(self):
        ...
        raw_orders = _load("kestrelby_orders_4500038166.edi")
...
class TestKestrelbyOrdrspIntegration(EDIKestrelbySetup, TransactionCase):
    """
    ...
    def _get_ordrsp(self, review) -> str:
        ...
        try:
            ...
            from mml_edi.parsers.kestrelby import KestrelbyParser
        return KestrelbyParser().generate_ack(review).decode("utf-8")
    ...
    def test_ordrsp_po_reference_present(self):
        """ORDRSP must contain RFF+ON:<po_number> referencing the original Kestrelby PO."""
        self.review_1005.write({"state": "approved"})
        ordrsp = self._get_ordrsp(self.review_1005)
        self.assertIn("RFF+ON:4500038166", ordrsp)
