"""Pure tests for the Nimbrel inbound pipeline fixes (Wave1-B / go-live gate
...
def _make_partner(edi_format="edifact_d01b", code="NIMBREL", **extra):
...
class TestEmitInboundContrl:
    ...
    def test_noop_when_parser_lacks_generate_contrl(self, monkeypatch):
        """Preserve Kestrelby byte-for-byte: a parser with no generate_contrl
...
class TestIsContrlMessage:
    ...
    def test_orders_message_is_not_contrl(self):
        proc = EDIProcessor()
        orders_raw = (
            "UNA:+.? 'UNB+UNOC:3+NIMBREL:ZZZ+SUPPLIER_GLN:14+200928:1030+1'"
...
class TestHandleInboundContrl:
    ...
    def test_positive_contrl_routes_to_parse_contrl_and_logs_received(self):
        def parse_contrl(raw_text):
            return {
                ...
                "original_recipient_id": "NIMBREL", "original_recipient_qual": "ZZZ",
            }
    ...
    def test_noop_false_when_parser_lacks_parse_contrl(self):
        """A parser without CONTRL support (Kestrelby) must be a strict no-op
...
class TestProcessFileRawDataAssignment:
    ...
    when the parser did not already set it — Kestrelby (which decodes and
    ...
    def test_kestrelby_style_parser_raw_data_preserved(self, monkeypatch):
        """Parser already decoded+set raw_data (Kestrelby idiom) — the
    ...
    def test_nimbrel_style_parser_gets_processor_fallback_decode(self, monkeypatch):
        """Parser leaves raw_data unset (Nimbrel idiom) — the processor must
...
_VALID_ORDERS_ENVELOPE = (
    "UNA:+.? 'UNB+UNOC:3+NIMBREL:ZZZ+SUPPLIER_GLN:14+200928:1030+1++++1'"
    "UNH+1+ORDERS:D:01B:UN:EAN008'BGM+220+PO123+9'UNT+3+1'UNZ+1+1'"
)
...
class TestValidateInboundEnvelope:
    ...
    def test_truncated_interchange_raises_parse_error(self):
        proc = EDIProcessor()
        partner = _make_partner(edi_format="edifact_d01b")
        truncated = "UNA:+.? 'UNB+UNOC:3+NIMBREL:ZZZ+SUPPLIER_GLN:14+200928:1030+1++++1'UNH+1+ORDERS:D:01B:UN:EAN008'"
    ...
    def test_sender_match_passes_when_c1_helper_present(self):
        proc = EDIProcessor()
        partner = _make_partner(edi_format="edifact_d01b")
        # Matches the NIMBREL:ZZZ sender baked into _VALID_ORDERS_ENVELOPE.
        partner.get_unb_recipient = lambda: ("NIMBREL", "ZZZ")
...
# ── Real-parser integration: the actual Nimbrel cancellation fixture ──────

class TestRealNimbrelCancellationFixture:
    """Feeds the real NimbrelParser's parsed cancellation output through
    ...
    def test_real_parsed_cancellation_routes_to_process_cancellation(self):
        ...
        fixture = (
            ...
            / "nimbrel_orders_cancel_PO0319333.edi"
        )
        raw = fixture.read_bytes()
        orders = NimbrelParser().parse_file(raw, trading_partner=None)
        assert len(orders) == 1
        order = orders[0]
