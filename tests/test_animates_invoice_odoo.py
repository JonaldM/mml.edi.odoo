# mml.edi/tests/test_animates_invoice_odoo.py
"""Odoo TransactionCase tests for the Animates INVOIC path (Wave2-E,
AN-03 INVOIC half / scenarios 1, 4B, 5A, 5B).

Covers what the pure mocked tests/test_animates_invoice_service.py cannot: a
...
class TestAnimatesInvoiceOdoo(EDITestSetup, TransactionCase):

    def setUp(self):
        super().setUp()
        self.setup_edi_test_data()
        self.test_product.is_storable = True
        # Animates matches on default_code (shared fixture only sets barcode) —
        ...
        # Animates prices are ex-GST (module convention) — attach a real 15%
        ...
        self.animates_pricelist = self.env["product.pricelist"].create({
            "name": "Animates Invoice Test Pricelist",
    ...
    def _confirm_order(self, po_number, store_code="12345", qty=10.0):
        ...
        self.processor.process_parsed_order(
            order, self.animates_partner, "animates_invoice_test.edi", file_hash)
    ...
    def test_invoice_payload_qty_matches_shipped_not_ordered(self):
        from odoo.addons.mml_edi.services.animates_invoice import build_invoic_payload_from_move
    ...
    def test_invoice_generates_and_uploads_invoic(self):
        from odoo.addons.mml_edi.services.animates_invoice import generate_and_upload_invoic

        so, review = self._confirm_order("PO-INVOICE-3", qty=4.0)
        self._ship(so)
        invoice = self._invoice(so)

        generate_and_upload_invoic(self.env, invoice, self.animates_partner)
        logs = self.env["edi.log"].search([
            ("trading_partner_id", "=", self.animates_partner.id),
            ("event_type", "=", "ack_sent"),
            ("filename", "like", "INVOIC_ANIMATES_%"),
        ])
        self.assertTrue(logs, "Animates INVOIC dispatch must log an ack_sent event")
    ...
    def test_invoice_envelope_identity_is_real_not_placeholder(self):
        from odoo.addons.mml_edi.parsers.animates_edifact import tokenize
        from odoo.addons.mml_edi.services.animates_invoice import generate_and_upload_invoic
        ...
        invoic_bytes = generate_and_upload_invoic(self.env, invoice, self.animates_partner)
        _, segs = tokenize(invoic_bytes.decode("latin-1"))
        unb = [s for s in segs if s.tag == "UNB"][0]
        self.assertEqual(unb.comp(1, 0), "9419416000008T")
...
class TestAnimatesInvoiceQtyContractPure(unittest.TestCase):
    ...
    def test_service_module_importable(self):
        try:  # real Odoo runtime
            from odoo.addons.mml_edi.services.animates_invoice import (
                AnimatesInvoiceError, build_invoic_payload_from_move,
                generate_and_upload_invoic, shipped_qty_by_sale_line,
            )
        except ImportError:  # pure-pytest conftest shim
            from mml_edi.services.animates_invoice import (
                AnimatesInvoiceError, build_invoic_payload_from_move,
                generate_and_upload_invoic, shipped_qty_by_sale_line,
            )
        ...
        self.assertTrue(issubclass(AnimatesInvoiceError, Exception))
