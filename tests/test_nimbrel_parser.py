"""Pure-Python tests for the Nimbrel ORDERS inbound parser (B2).

No Odoo env. Run: pytest tests/test_nimbrel_parser.py -q
"""
...
def _orders():
    ...
    return NimbrelParser().parse_file(raw, trading_partner=None)
...
def test_str_input_also_accepted():
    raw = (FIXTURES / "nimbrel_orders_PO169603.edi").read_text(encoding="iso-8859-1")
...
def test_no_unh_raises():
    ...
    with pytest.raises(EDIParseError):
        NimbrelParser().parse_file(b"GARBAGE+not+edifact'", trading_partner=None)


# --- C5 / AN-05: cancellation (BGM 1225=1, Testing Scenario Handbook 3B) ---

def _cancel_order():
    raw = (FIXTURES / "nimbrel_orders_cancel_PO0319333.edi").read_bytes()
    orders = NimbrelParser().parse_file(raw, trading_partner=None)
    assert len(orders) == 1
    return orders[0]
...
def test_change_order_is_still_change_order_not_cancellation():
    ...
    o = NimbrelParser().parse_file(changed, trading_partner=None)[0]
