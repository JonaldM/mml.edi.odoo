"""Pure-Python tests for the embedded Animates store master table (Wave2-F,
gate review AN-13/OPS-5).

Covers: table integrity against the source xlsx row/dedup count, ref format
compatibility with what parsers/animates.py extracts from NAD+ST, and the
clinic/retail collision-collapse documented in
wizards/animates_store_master_data.py.

No Odoo. Run: python -m pytest tests/test_animates_store_master.py -q
"""
import re

import pytest

from mml_edi.wizards.animates_store_master_data import (
    _ANIMATES_STORES,
    get_animates_stores,
)
...
def test_get_animates_stores_returns_expected_unique_count():
    stores = get_animates_stores()
    assert len(stores) == _EXPECTED_UNIQUE_COUNT


def test_get_animates_stores_returns_tuple_of_tuples_no_io():
...
def test_no_duplicate_refs_in_embedded_table():
    ...
    codes = [code for code, _name, _region in _ANIMATES_STORES]
...
def test_no_blank_code_or_name():
    for code, name, _region in _ANIMATES_STORES:
...
def test_no_bom_or_stray_whitespace_in_names():
    ...
    for code, name, region in _ANIMATES_STORES:
        assert "﻿" not in name
...
def test_all_codes_match_known_formats():
    ...
    for code, _name, _region in _ANIMATES_STORES:
...
def test_numeric_codes_are_never_int_cast_leading_zero_preserved():
    ...
    numeric_codes = [c for c, _n, _r in _ANIMATES_STORES if _NUMERIC_CODE_RE.match(c)]
    assert "02" in numeric_codes
    assert all(isinstance(c, str) for c in numeric_codes)


def test_retail_codes_present():
    retail_codes = {code for code, _n, _r in _ANIMATES_STORES if code.startswith("R-")}
...
def _nad_st_store_code(nad_line: str) -> str:
    """Mirror parsers/animates.py's NAD+ST handling: seg.comp(1, 0), i.e.
...
def test_embedded_ref_format_matches_nad_st_extraction(code):
    ...
    unknown_store."""
    nad_line = "NAD+ST+%s::92++Animates Test Store+1 Test Street+Testville++0000+NZ" % code
    extracted = _nad_st_store_code(nad_line)
    assert extracted == code

    codes_in_table = {c for c, _n, _r in _ANIMATES_STORES}
