...
def test_check_digit_matches_real_animates_desadv_pallet_sscc():
    """00593161000045350112 (AI 00 + 18-digit SSCC) from Animates_DESADV.pdf p.56
...
def test_check_digit_matches_real_animates_desadv_carton_sscc():
...
def test_build_sscc18_correct_length_and_structure():
    sscc = build_sscc18("9419416", 1)
    assert len(sscc) == 18
    assert sscc.isdigit()
    assert sscc.startswith("0" + "9419416")  # default extension digit '0'


def test_build_sscc18_is_valid():
    sscc = build_sscc18("9419416", 42)
...
def test_build_sscc18_deterministic_check_digit():
    a = build_sscc18("9419416", 555)
    b = build_sscc18("9419416", 555)
    assert a == b


def test_build_sscc18_different_serials_differ():
    a = build_sscc18("9419416", 1)
    b = build_sscc18("9419416", 2)
    assert a != b


def test_build_sscc18_custom_extension_digit():
    sscc = build_sscc18("9419416", 1, extension_digit="7")
    assert sscc.startswith("7" + "9419416")


def test_build_sscc18_max_serial_for_7digit_prefix():
    # 9 serial digits available -> max is 999999999.
    sscc = build_sscc18("9419416", 999999999)
    assert validate_sscc18(sscc) is True


def test_build_sscc18_serial_overflow_raises():
    with pytest.raises(GS1SSCCError):
        build_sscc18("9419416", 10 ** 9)  # 10 digits, one too many
...
def test_build_sscc18_rejects_negative_serial():
    with pytest.raises(GS1SSCCError):
        build_sscc18("9419416", -1)


def test_build_sscc18_rejects_bad_extension_digit():
    with pytest.raises(GS1SSCCError):
        build_sscc18("9419416", 1, extension_digit="10")
    with pytest.raises(GS1SSCCError):
        build_sscc18("9419416", 1, extension_digit="")
