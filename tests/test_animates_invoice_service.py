# mml.edi/tests/test_animates_invoice_service.py
"""Pure-Python tests for services.animates_invoice (Wave2-E, AN-03 INVOIC half).
...
another mock). Run: python -m pytest tests/test_animates_invoice_service.py -q
...
class FakeProduct:
    def __init__(self, default_code="MML-001", display_name="Test Product",
        ...
        # Animates ISC (buyer item code) — MML stores it in x_articleno.
...
class FakePartner:
    def __init__(self, name="Animates NZ Holding LTD", street="", city="",
...
def test_payload_buyer_nzbn_from_config_param_overrides_partner_vat():
    move, sol, order, picking, move_line = _basic_setup()
    move.env = FakeEnv(
        config_values={"mml_edi.animates_nzbn": "9999999999999"},
