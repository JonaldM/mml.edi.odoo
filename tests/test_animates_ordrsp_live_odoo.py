# mml.edi/tests/test_animates_ordrsp_live_odoo.py
"""Odoo TransactionCase tests for AnimatesParser.generate_ack / _review_to_
...
Complements tests/test_animates_ordrsp_live.py (pure, duck-typed
...
_ANIMATES_ORDERS_TWO_LINES = (
    "UNA:+.? '"
    "UNB+UNOC:3+ANIMATES:ZZZ+SUPPLIER_GLN:14+260703:0900+55001++++1'"
...
class _AnimatesOrdrspLiveOdooBase(EDITestSetup, TransactionCase):
    """Shared setup: an Animates (EDIFACT D.01B) trading partner with real C1
    ...
    def _make_review(self, so, po="PO-LIVE-001", file_hash="livehash0000001",
        ...
        return self.env["edi.order.review"].create({
            ...
            "edi_raw_data": _ANIMATES_ORDERS_TWO_LINES,
            "document_type": "new_order",
            "state": state,
        })

    def _segs(self, review):
        from odoo.addons.mml_edi.parsers import animates_edifact as edifact
...
class TestGenerateAckLiveQtyOdoo(_AnimatesOrdrspLiveOdooBase):
...
class TestGenerateAckScenario4APriceCorrectionOdoo(_AnimatesOrdrspLiveOdooBase):
...
class TestGenerateAckSS6FailClosedOdoo(_AnimatesOrdrspLiveOdooBase):
...
class TestGenerateAckEnvelopeIdentityOdoo(_AnimatesOrdrspLiveOdooBase):
    ...
    def test_production_environment_recipient_is_animates(self):
        ...
        segs = self._segs(review)
        unb = self._seg(segs, "UNB")[0]
        self.assertEqual(unb.elements[2][0], "ANIMATES")


class TestGenerateAckCancellationRefusalOdoo(_AnimatesOrdrspLiveOdooBase):
    ...
    def test_refuses_review_with_cancellation_marker_change_summary(self):
        from odoo.addons.mml_edi.parsers.animates import CancellationAckRefused
