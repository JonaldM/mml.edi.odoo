"""Pure-Python tests for the Animates D.01B EDIFACT foundation (B1).

No Odoo env. Run: pytest tests/test_animates_edifact.py -q
"""
...
def test_unb_business_uses_supplier14_animates_zzz_with_tail():
    unb = build_unb("5412345000013", 12341, "200916", "1030")
    assert unb.elements[1] == ["5412345000013", "14"]   # sender = supplier:14
    assert unb.elements[2] == ["ANIMATES", "ZZZ"]        # recipient = ANIMATES:ZZZ
...
def test_unb_inbound_inverts_parties():
    ...
    assert unb.elements[1] == ["ANIMATES", "ZZZ"]
...
def test_serialize_round_trip_structural():
    src = _load("animates_orders_PO169603.edi")
    _, segs = tokenize(src)
...
@pytest.mark.parametrize("name", [
    "animates_orders_PO169603.edi", "animates_ordrsp_expected.edi",
    "animates_invoic_expected.edi", "animates_contrl_in.edi", "animates_desadv_split.edi", "animates_desadv_pallet.edi",
])
def test_fixture_envelope_is_animates_shaped(name):
...
def test_contrl_fixture_unb_has_no_trailing_tail():
    _, segs = tokenize(_load("animates_contrl_in.edi"))
    unb = [s for s in segs if s.tag == "UNB"][0]
