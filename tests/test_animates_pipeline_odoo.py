# mml.edi/tests/test_animates_pipeline_odoo.py
"""
Odoo TransactionCase tests for the Animates inbound pipeline fixes
...
Complements tests/test_animates_pipeline.py (pure, no Odoo). These exercise
...
_ANIMATES_ORDERS = (
    "UNA:+.? '"
    "UNB+UNOC:3+ANIMATES:ZZZ+SUPPLIER_GLN:14+210114:0915+88301++++1'"
    ...
    "NAD+ST+12345::92++Animates Invercargill'"
    "LIN+1'"
    "PIA+5+ISC001:IN'"
...
_ANIMATES_CANCEL = (
    "UNA:+.? '"
    "UNB+UNOC:3+ANIMATES:ZZZ+SUPPLIER_GLN:14+210114:0915+88302++++1'"
    "UNH+1+ORDERS:D:01B:UN:EAN011'"
...
class _AnimatesOdooTestBase(EDITestSetup, TransactionCase):
    """Shared setup: an Animates-format (EDIFACT D.01B) trading partner with
    the real AnimatesParser allow-listed, FTP replaced by a recording fake."""
...
class TestLatin1IngestOdoo(_AnimatesOdooTestBase):
    ...
    def test_review_raw_data_preserves_latin1_byte(self):
        raw_with_latin1 = _ANIMATES_ORDERS.replace(
...
@mute_logger("odoo.addons.mml_edi.models.edi_processor",
             "odoo.addons.mml_edi.models.edi_order_review")
class TestContrlEmissionOdoo(_AnimatesOdooTestBase):
    ...
    def _stub_generate_contrl(self):
        """The real AnimatesParser.generate_contrl may not exist yet on this
        ...
        exactly once).
        """
        from odoo.addons.mml_edi.parsers.animates import AnimatesParser
        patcher = mock.patch.object(
            AnimatesParser, "generate_contrl",
    ...
    def test_contrl_uploaded_after_processing_orders_file(self):
        ...
        failures = self.processor._process_file(
            _ANIMATES_ORDERS.encode("iso-8859-1"), "hash-contrl-1",
    ...
    def test_contrl_not_duplicated_on_repeated_send_file_responses(self):
        ...
        failures = self.processor._process_file(
            _ANIMATES_ORDERS.encode("iso-8859-1"), "hash-contrl-2",
    ...
    def test_no_contrl_support_is_a_strict_noop_briscoes(self):
        """Preserve Briscoes byte-for-byte: the iDOC parser has no
        ...
        raw = make_idoc_raw("PO-BRISCOES-1").encode("utf-8")
        failures = self.processor._process_file(
            raw, "hash-briscoes-1", "idoc1.xml", self.trading_partner,
        )
        self.assertEqual(failures, [])
        self.processor._send_file_responses(self.trading_partner, "hash-briscoes-1")
...
@mute_logger("odoo.addons.mml_edi.models.edi_processor")
class TestInboundContrlRoutingOdoo(_AnimatesOdooTestBase):
    ...
    def _stub_parse_contrl(self):
        from odoo.addons.mml_edi.parsers.animates import AnimatesParser
        from odoo.addons.mml_edi.parsers.animates_contrl import parse_contrl as real_parse_contrl
        patcher = mock.patch.object(
...
@mute_logger("odoo.addons.mml_edi.models.edi_processor",
             "odoo.addons.mml_edi.models.edi_order_review")
class TestCancellationRoutingOdoo(_AnimatesOdooTestBase):
    ...
    def _create_confirmed_so(self, po="PO-ODOO-001"):
        failures = self.processor._process_file(
            _ANIMATES_ORDERS.encode("iso-8859-1"), "hash-orig-1",
    ...
    def test_cancellation_cancels_so_and_creates_terminal_review(self):
        ...
        failures = self.processor._process_file(
            _ANIMATES_CANCEL.encode("iso-8859-1"), "hash-cancel-1",
    ...
    def test_cancellation_never_queues_ordrsp(self):
        ...
        failures = self.processor._process_file(
            _ANIMATES_CANCEL.encode("iso-8859-1"), "hash-cancel-2",
    ...
    def test_retry_pending_acks_never_touches_cancellation_review(self):
        self._create_confirmed_so()
        failures = self.processor._process_file(
            _ANIMATES_CANCEL.encode("iso-8859-1"), "hash-cancel-3",
    ...
    def test_cancel_before_original_logs_warning_creates_no_review(self):
        ...
        failures = self.processor._process_file(
            _ANIMATES_CANCEL.encode("iso-8859-1"), "hash-cancel-early",
