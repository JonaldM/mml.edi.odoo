"""Pure tests for NimbrelParser._review_to_ordrsp_payload / generate_ack against
...
Run: pytest tests/test_nimbrel_ordrsp_live.py -q
"""
...
from mml_edi.parsers.nimbrel import NimbrelParser, _review_to_ordrsp_payload
...
def test_generate_ack_prod_recipient_is_nimbrel():
    review = _review(partner=_partner(environment="production"))
    out = NimbrelParser().generate_ack(review)
    segs = _segs_from_bytes(out)
    unb = _seg(segs, "UNB")[0]
    assert unb.elements[2][0] == "NIMBREL"


def test_generate_ack_supplier_nad_uses_nimbrel_vendor_code_not_partner_code():
    """NAD+SU must carry the Nimbrel-assigned vendor code (C1
    nimbrel_vendor_code), not our internal partner.code."""
...
def test_generate_ack_buyer_nad_derived_from_partner_recipient():
    ...
    assert nad_by.comp(1, 0) == "TST1NIMBREL"
