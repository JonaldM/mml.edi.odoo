# mml.edi/services/animates_invoice.py
"""account.move -> Animates D.01B INVOIC builder (Wave2-E, AN-03 INVOIC half /
scenarios 1, 4B, 5A, 5B).

Pure-ish glue: turns a posted ``account.move`` (customer invoice) into the
``parsers.animates_invoic.build_invoic`` payload dict and (optionally)
...
scenarios 4B/5A/5B, verbatim from Animates_INVOIC.pdf p.20 RFF notes and the
...
animates uses for the DESADV itself -- the picking's ``done`` stock.move
...
GST semantics (see the corrected docstring in parsers/animates_invoic.py):
...
``_dispatch_animates`` pair is picking-triggered (DESADV only) and has no
...
(env, move, partner) shape ``_dispatch_animates`` takes (picking, sale_order,
...
# A fixed/group/division tax on an Animates invoice line would make the TAX
...
def shipped_qty_by_sale_line(sale_order) -> dict:
    ...
    ``sale_line_ids`` -- edi_line_number is an Animates-specific identifier
...
def _line_tax_rate(move_line) -> str:
    ...
    Fail-closed: raises AnimatesInvoiceError if the line has no tax, more
    ...
    if not taxes:
        raise AnimatesInvoiceError(
            ...
            "Animates order." % (move_line.name or move_line.id)
        )
    if len(taxes) != 1:
        raise AnimatesInvoiceError(
            "Invoice line '%s' has %d taxes — Animates INVOIC (TAX 7+GST) "
...
def _product_isc(product):
    """The Animates ISC (buyer item code, PIA+5:IN) persisted on a product.
    ...
    ORDERS, which are authoritative for the ISC Animates actually sent.
    """
...
def _isc_map_from_orders(sale_order, partner):
    ...
    the SO — the authoritative source of Animates' item number (PIA+5:IN),
    which is NOT persisted on the SO line (mirrors the DESADV path's
    _animates_item_identity_map). Returns {} on any failure (no review, no raw
...
def build_invoic_payload_from_move(move, partner, *, isc_by_line=None) -> dict:
    """Build the ``parsers.animates_invoic.build_invoic`` payload dict from a
    ...
    Args:
        ...
        partner: edi.trading.partner record for Animates (used for
        ...
        isc_by_line: optional {sale_order_line: isc} override map (buyer
            ...
            edi_service.py::_pack_units_for_animates).

    Returns:
        Payload dict ready for ``parsers.animates_invoic.build_invoic``, or
        raises AnimatesInvoiceError if nothing on the invoice is shippable
    ...
    for idx, move_line in enumerate(
        ...
        if not sol:
            _logger.warning(
                "Animates INVOIC: invoice %s line '%s' has no linked sale "
        ...
        if qty_shipped <= 1e-6:
            _logger.info(
                "Animates INVOIC: invoice %s line '%s' (SO line %s) has "
        ...
        # Buyer item code (PIA+5:IN) MUST be Animates' ISC — never the LIN line
        ...
        # (x_articleno, where MML stores Animates ISCs). Fail CLOSED rather than
        # put a wrong identifier on the wire that Animates would reject/mis-post.
        isc = isc_by_line.get(sol) or _product_isc(move_line.product_id)
        if not isc:
            raise AnimatesInvoiceError(
                "No Animates ISC (buyer item code) for product %s on move line "
    ...
    if not lines:
        raise AnimatesInvoiceError(
            "Animates INVOIC: invoice %s has no lines with shipped quantity "
    ...
    if not ref_source["advice_no"] and not ref_source["connote"]:
        raise AnimatesInvoiceError(
            "Animates INVOIC: invoice %s — could not resolve a despatch "
            "advice number or carrier connote from any shipped line's "
            "picking (Animates requires at least one, per "
            "Animates_INVOIC.pdf RFF notes)." % move.name
        )
...
def _buyer_nzbn(env, buyer_partner) -> str:
    """Animates' NZBN for RFF+AMT under NAD+BY. Config-first (per-tenant
    ...
    configured = env["ir.config_parameter"].sudo().get_param("mml_edi.animates_nzbn")
    if configured:
        return configured
...
def generate_and_upload_invoic(env, move, partner) -> bytes:
    """Build, render, upload and log an Animates INVOIC for a posted
    account.move. Returns the rendered interchange bytes.

    Mirrors services.edi_service.EDIService._generate_and_upload_desadv_
    animates's shape exactly (real UNB identity via C1/C3, a fresh ctrl_ref
    ...
    'DESADV_ANIMATES_<po>_<date>' convention so the two are trivially
    distinguishable and searchable the same way
    (test_edi_service_animates_odoo.py's own
    ``filename like 'DESADV_ANIMATES_%'`` pattern).
    """
    ...
    move.message_post(body="INVOIC sent to Animates: %s" % filename)
    ...
    _logger.info(
        "EDI INVOIC: uploaded Animates INVOIC %s (%d bytes)",
