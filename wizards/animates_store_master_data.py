# mml.edi/wizards/animates_store_master_data.py
"""
Animates NZ clinic/store master data, embedded as a static Python table.

GENERATED from docs/animates/Animates-Clinic-Store-Master-File-12112025.xlsx
...
replacing _ANIMATES_STORES below; keep the row count / dedup logic in this
...
2. The source file lists 66 site rows across 7 regions, but 10 of those
   ...
   NAD+ST in an inbound Animates ORDERS carries a single store code with no
   second qualifier to disambiguate retail vs. clinic (parsers/animates.py
...
def get_animates_stores():
    """Return the embedded Animates store table as a tuple of
    (store_code, store_name, region) tuples. No I/O, no openpyxl — pure
    in-memory data, safe to call at runtime."""
    return tuple(_ANIMATES_STORES)
